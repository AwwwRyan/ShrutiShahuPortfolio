...
**Refinement round 4 (site-wide decorative system + Services layout):**
- **Added a site-wide decorative visual system** (`src/components/decor/`): a single hand-drawn-feeling bezier line weaving the full height of every page, drawing itself in as the user scrolls (stroke-dashoffset tied to scroll progress), plus a small reusable set of accent motifs (organic blobs, four-point sparkles, thin rings, a dot grid) placed sparingly — one or two per section, several sections left bare on purpose. Motifs get a subtle mouse-parallax drift, disabled outright on touch devices and under `prefers-reduced-motion`. Both sit behind all content (`pointer-events: none`, no z-index tricks — see memory.md for why) so they never interfere with clicking or reading.
- **Services section rebuilt as a responsive bento grid** (asymmetric cell sizes — one large featured cell, one tall cell, several regular ones) after a reference mockup. Kept to the mockup's *layout pattern* only — the reference also showed per-service description copy and a "MOST REQUESTED" badge, both left out since neither is previously-approved real content (the 6 service names are still the only Locked content here).
