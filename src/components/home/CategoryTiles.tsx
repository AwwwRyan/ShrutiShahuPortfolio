...
// Static decorative background photos per top-level category — not admin-editable content
// (Category has no `image` field; adding one would be a schema change), just fixed site assets.
...
export function CategoryTiles({ categories }: { categories: Category[] }) {
  return (
    <section id="categories" className="mx-auto max-w-6xl scroll-mt-24 px-6 py-20">
      <h2 className="font-serif text-3xl text-paper sm:text-4xl">Work</h2>

      {categories.length === 0 ? (
        <p className="mt-6 text-paper/60">Categories coming soon.</p>
      ) : (
